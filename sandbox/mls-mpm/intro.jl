...
mutable struct MPM
    ...
    function MPM(;
        ...
        grid = Vector{Cell}(undef, num_cells)
...
function particles_to_grid!(mpm::MPM)
    for i in 1:mpm.num_particles
        ...
        for gx in 0:2, gy in 0:2
            ...
            # Transform 2D position to 1D grid index to retrieve cell.
            grid_idx = Int32(cell_position[1]) * mpm.grid_resolution + Int32(cell_position[2])
            cell = mpm.grid[grid_idx]
            ...
            mpm.grid[grid_idx] = Cell(cell_velocity, cell_mass)
        end
    end
end

function grid_velocity_update!(mpm::MPM)
    for i in 1:mpm.num_cells
        cell = mpm.grid[i]
        ...
        # Boundary conditions.
        x = i / mpm.grid_resolution
        y = i % mpm.grid_resolution
        !(2 < x < mpm.grid_resolution - 3) && (cell_velocity *= Vec2f0(0f0, 1f0))
        !(2 < y < mpm.grid_resolution - 3) && (cell_velocity *= Vec2f0(1f0, 0f0))

        mpm.grid[i] = Cell(cell_velocity, cell.mass)
    end
end
...
function grid_to_particles!(mpm::MPM)
    for i in 1:mpm.num_particles
        ...
        for gx in 0:2, gy in 0:2
            ...
            # Transform 2D position to 1D grid index to retrieve cell.
            grid_idx = Int32(cell_position[1]) * mpm.grid_resolution + Int32(cell_position[2])
            weighted_velocity = mpm.grid[grid_idx].velocity * weight
            ...
            particle_velocity = particle.velocity + weighted_velocity
        end
        particle_affine_momentum = affine_momentum .* 4

        # Advect particle.
        particle_position = particle.position + particle_velocity * mpm.δt
        particle_position = clamp.(particle_position, 1.5f0, mpm.grid_resolution - 2)
        # TODO Handle mouse interactions.
...
function simulate!(mpm::MPM)
    # Reset grid.
    for i in 1:mpm.num_cells
        mpm.grid[i] = Cell(Vec2f0(0f0, 0f0), 0f0)
    end
...
function MPMLayer()
    ...
    mpm = MPM()
    MPMLayer(controller, mpm)
end

function draw_particles(
    mpm::MPM;
    particle_size::Float32 = 0.01f0, max_velocity::Float32 = 4f0,
    base_color = Point4f0(0.2f0, 0.3f0, 0.8f0, 1f0),
    top_color = Point4f0(1f0),
)
    # TODO draw grid boundaries
    # TODO remove gravity influence when calculating color
    for particle in mpm.particles
        world_position = (
            particle.position ./ Float32(mpm.grid_resolution) .- 0.5f0
        )
        σ = clamp(norm(particle.velocity) / max_velocity, 0f0, 1f0)
