...
@with_kw mutable struct Renderer2DData
    ...
    single_color_shader::Union{Backend.Shader, Nothing} = nothing
end

const Data = Renderer2DData()

function init()
    layout = BufferLayout([BufferElement(Point3f0, "a_Position")])
    data = Float32[
        -0.5, -0.5, 0.0,
         0.5, -0.5, 0.0,
         0.5,  0.5, 0.0,
    ...
    Data.single_color_shader = Backend.Shader(
...
function begin_scene(camera::OrthographicCamera)
    Data.single_color_shader |> Backend.bind
    Backend.upload_uniform(
        Data.single_color_shader, "u_ViewProjection", camera.view_projection,
    )
end
...
function draw_quad(position::Vec3f0, size::Vec2f0, color::Vec4f0)
    Data.single_color_shader |> Backend.bind
    Backend.upload_uniform(Data.single_color_shader, "u_Color", color)

    transformation = translation(position) * scaling(size[1], size[2], 1f0)
    Backend.upload_uniform(Data.single_color_shader, "u_Transform", transformation)
